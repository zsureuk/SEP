...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        # self.fc2 = nn.Linear(in_features=round(planes / 4), out_features=round(planes / 2))
        # self.fc2 = nn.Linear(in_features=round(planes / 4), out_features=planes)
        # self.fc2_cross_layer = nn.Linear(in_features=round(planes / 4), out_features=round(planes / 2))
    ...
    def forward(self, x):
        ...
        # sep_threshold = torch.from_numpy(np.array(sep_threshold_np)).cuda()
        # print('sep',sep)
...
def se_resnet44(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SEP(Bottleneck, [7, 7, 7], **kwargs)
    return model
...
def se_resnet56(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SEP(Bottleneck, [9, 9, 9], **kwargs)
    return model
...
def se_resnet110(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SEP(Bottleneck, [18, 18, 18], **kwargs)
    return model
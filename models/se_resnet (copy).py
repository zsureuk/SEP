    ...
    def forward(self, x):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            if out.size(1) == x.size(1):
                ...
                # sep_vec = sep.cpu().detach().numpy()
                print('sep_vec',sep_vec.shape)
                # sep_vec = sep.view(sep.size(0),-1).cpu().detach().numpy()
                sep_sort = np.sort(sep_vec)
                for i in range(len(sep_sort)):
                    sep_threshold = sep_sort[i][int(self.prune_rate*len(sep_sort[i]))]
                    # self.sep_threshold.append(sep_sort[i][int(self.prune_rate*len(sep_sort[i]))])
                    sep[i] = self.relu(sep[i]-sep_threshold)
